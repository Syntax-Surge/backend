...
const getAllOrderItems = async (call, callback) => {


    try {

        const order1 = await orders.findAll({
            attributes: ['userId', 'shippingMethod', 'status', 'id'],
            include: [
                {
                    model: OrderItems,
                    as: 'items', // Alias defined in the association
...
server.addService(orderItemsProto.UserService.service, { GetOrderItemsById: getOrderItemsById, GetAllOrderItems: getAllOrderItems });
